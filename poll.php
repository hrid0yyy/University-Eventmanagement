...
<body> 
  ...
  <?php
       ...
       while($row = mysqli_fetch_assoc($res))
       {
          ...
          <td><a href='pollresult.php?pid=". $row['PollID'] . "&oid=".$oid ."&ename=".$row['EventName']."'>Result</a></td>
          <td> <button class='delete btn btn-sm btn-primary' id=d".$row['PollID'].">Delete</button></td>
...
<script>
    ...
    console.log("edit ", );
   PollID = e.target.id.substr(1,);
    console.log(PollID);
    
    if(confirm("Press yes to delete!"))
    {
      console.log("yes");
      window.location=`/university/deletepoll.php?delete=${PollID}&oid=<?php echo $oid ?>`;
    }
    else
    {
      console.log("No");
